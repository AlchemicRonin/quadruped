...
from dribblebot.utils.math_utils import get_scale_shift, quat_apply_yaw, wrap_to_pi
...
class LeggedRobot(BaseTask):
        ...
        for _ in range(self.cfg.control.decimation):
            ...
            if self.ball_force_feedback is not None:
                ...
                if asset_forces is not None:
                    self.forces[
                        :, self.num_bodies : self.num_bodies + self.num_object_bodies
                    ] = asset_forces
    ...
    def post_physics_step(self):
        ...
        if self.cfg.env.add_balls:
            ...
            self.object_pos_world_frame[:] = self.root_states[
                self.object_actor_idxs, 0:3
            ]
            robot_object_vec = self.asset.get_local_pos()
            true_object_local_pos = quat_rotate_inverse(
                self.base_quat, robot_object_vec
            )
            true_object_local_pos[:, 2] = 0.0 * torch.ones(
                self.num_envs,
                dtype=torch.float,
                device=self.device,
                requires_grad=False,
            ...
            self.object_local_pos = self.simulate_ball_pos_delay(
                true_object_local_pos, self.object_local_pos
            )
    ...
    def randomize_ball_state(self):
        reset_ball_pos_mark = np.random.choice(
            [True, False],
            self.num_envs,
            p=[self.cfg.ball.pos_reset_prob, 1 - self.cfg.ball.pos_reset_prob],
        )
        ...
        ball_pos_env_ids = self.object_actor_idxs[reset_ball_pos_env_ids].to(
            device=self.device
        )
        ...
        self.root_states[ball_pos_env_ids, 0:3] += (
            ...
            * torch.tensor(
                self.cfg.ball.pos_reset_range, device=self.device, requires_grad=False
            )
        )
        ...
        reset_ball_vel_mark = np.random.choice(
            [True, False],
            self.num_envs,
            p=[self.cfg.ball.vel_reset_prob, 1 - self.cfg.ball.vel_reset_prob],
        )
        reset_ball_vel_env_ids = torch.tensor(
            np.array(np.nonzero(reset_ball_vel_mark)), device=self.device
        ).flatten()
        ball_vel_env_ids = self.object_actor_idxs[reset_ball_vel_env_ids].to(
            device=self.device
        )
        self.root_states[ball_vel_env_ids, 7:10] = (
            ...
            * torch.tensor(
                self.cfg.ball.vel_reset_range, device=self.device, requires_grad=False
            )
        )
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            gymtorch.unwrap_tensor(
                torch.cat((reset_ball_pos_env_ids_int32, reset_ball_vel_env_ids_int32))
            ),
            len(reset_ball_pos_env_ids_int32) + len(reset_ball_vel_env_ids_int32),
        )
    ...
    def simulate_ball_pos_delay(self, new_ball_pos, last_ball_pos):
        receive_mark = np.random.choice(
            ...
            self.num_envs,
            p=[
                self.cfg.ball.vision_receive_prob,
                1 - self.cfg.ball.vision_receive_prob,
            ],
        )
    ...
    def check_termination(self):
        ...
        if self.cfg.rewards.use_terminal_body_height:
            self.body_height_buf = (
                torch.mean(
                    self.root_states[self.robot_actor_idxs, 2].unsqueeze(1)
    ...
    def reset_idx(self, env_ids):
        ...
        self.path_distance[env_ids] = 0.0
    ...
    def set_idx_pose(self, env_ids, dof_pos, base_state):
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            gymtorch.unwrap_tensor(
                torch.cat(
                    (
                        self.robot_actor_idxs[env_ids_long],
                        self.object_actor_idxs[env_ids_long],
                    )
                )
            ),
            2 * len(env_ids_int32),
    ...
    def compute_reward(self):
        ...
        if self.cfg.rewards.only_positive_rewards:
            ...
            self.rew_buf[:] = self.rew_buf_pos[:] * torch.exp(
                self.rew_buf_neg[:] / self.cfg.rewards.sigma_rew_neg
            )
        self.episode_sums["total"] += self.rew_buf
        # add termination reward after clipping
        if "termination" in self.reward_scales:
            rew = (
                self.reward_container._reward_termination()
        ...
        self.command_sums["lin_vel_residual"] += (
            self.base_lin_vel[:, 0] - self.commands[:, 0]
        ) ** 2
        self.command_sums["ang_vel_residual"] += (
            ...
            if sensor_name in ALL_SENSORS.keys():
                self.sensors.append(
                    ALL_SENSORS[sensor_name](
                        self, **self.cfg.sensors.sensor_args[sensor_name]
                    )
                )
            else:
        ...
        for privileged_sensor_name in self.cfg.sensors.privileged_sensor_names:
            if privileged_sensor_name in ALL_SENSORS.keys():
                self.privileged_sensors.append(
                    ALL_SENSORS[privileged_sensor_name](
                        self,
                        **self.cfg.sensors.privileged_sensor_args[
    ...
    def compute_observations(self):
        ...
        self.pre_obs_buf = torch.reshape(
            torch.cat(self.pre_obs_buf, dim=-1), (self.num_envs, -1)
        )
        ...
        self.privileged_obs_buf = torch.reshape(
            torch.cat(self.privileged_obs_buf, dim=-1), (self.num_envs, -1)
        )
        # add noise if needed
    ...
    def create_sim(self):
        ...
        if mesh_type not in ALL_TERRAINS.keys():
            raise ValueError(
                f"Terrain mesh type {mesh_type} not recognised. Allowed types are {ALL_TERRAINS.keys()}"
            )
    ...
    def _randomize_gravity(self, external_force=None):
        if external_force is not None:
            ...
            external_force = (
                torch.rand(
                    3, dtype=torch.float, device=self.device, requires_grad=False
                )
    ...
    def _apply_drag_force(self, force_tensor):
        if self.cfg.domain_rand.randomize_ball_drag:
            force_tensor[:, self.num_bodies, :2] = (
                -self.ball_drags
                * torch.square(self.object_lin_vel[:, :2])
                * torch.sign(self.object_lin_vel[:, :2])
            )
        ...
        if cfg.env.add_balls:
            if cfg.domain_rand.randomize_ball_restitution:
                min_restitution, max_restitution = (
                    cfg.domain_rand.ball_restitution_range
                )
                self.ball_restitutions[env_ids] = (
                    torch.rand(
                        len(env_ids),
                        1,
                        dtype=torch.float,
            ...
            if cfg.domain_rand.randomize_ball_friction:
                ...
                self.ball_friction_coeffs[env_ids] = (
                    torch.rand(
                        len(env_ids),
                        1,
                        dtype=torch.float,
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.commands.heading_command:
            ...
            self.commands[:, 2] = torch.clip(
                0.5 * wrap_to_pi(self.heading_commands - heading), -1.0, 1.0
            )
        ...
        env_ids = (
            (self.episode_length_buf % int(self.cfg.domain_rand.rand_interval) == 0)
            .nonzero(as_tuple=False)
            .flatten()
        )
        self._randomize_dof_props(env_ids, self.cfg)
        ...
        if (
            self.common_step_counter % int(self.cfg.domain_rand.ball_drag_rand_interval)
            == 0
        ):
    ...
    def _resample_commands(self, env_ids):
        ...
        for i, (category, curriculum) in enumerate(
            ...
            if (
                isinstance(env_ids_in_category, np.bool_)
                or len(env_ids_in_category) == 1
            ):
    ...
    def _compute_torques(self, actions):
        ...
        actions_scaled[:, : self.num_actuated_dof] = (
            actions[:, : self.num_actuated_dof] * self.cfg.control.action_scale
        )
        if self.num_actions >= 12:
            actions_scaled[:, [0, 3, 6, 9]] *= (
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        random_yaw_angle = (
            ...
            * torch.tensor([0, 0, cfg.terrain.yaw_init_range], device=self.device)
        )
        self.root_states[robot_env_ids, 3:7] = quat_from_euler_xyz(
            random_yaw_angle[:, 0], random_yaw_angle[:, 1], random_yaw_angle[:, 2]
        )

        # base velocities
        self.root_states[robot_env_ids, 7:13] = torch_rand_float(
            -0.5, 0.5, (len(robot_env_ids), 6), device=self.device
        ...
        if self.cfg.env.add_balls:
            ...
            self.root_states[object_env_ids, 0:3] += (
                ...
                * torch.tensor(
                    cfg.ball.init_pos_range, device=self.device, requires_grad=False
                )
            )
            self.root_states[object_env_ids, 7:10] += (
                2
                * (
                    torch.rand(
                        len(env_ids),
                ...
                * torch.tensor(
                    cfg.ball.init_vel_range, device=self.device, requires_grad=False
                )
            )
    ...
    def _push_robots(self, env_ids, cfg):
        ...
        if cfg.domain_rand.push_robots:
            env_ids = env_ids[
                self.episode_length_buf[env_ids] % int(cfg.domain_rand.push_interval)
                == 0
            ]

            max_vel = cfg.domain_rand.max_push_vel_xy
            self.root_states[self.robot_actor_idxs[env_ids], 7:9] = torch_rand_float(
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        if not self.init_done:
            ...
            return
        distance = torch.norm(
            self.root_states[robot_env_ids, :2] - self.env_origins[env_ids, :2], dim=1
        )
        # robots that walked far enough progress to harder terains
        move_up = distance > self.terrain.cfg.env_length / 2

        move_down = (
            self.path_distance[env_ids]
        ...
        self.terrain_levels[env_ids] = torch.where(
            ...
            torch.clip(self.terrain_levels[env_ids], self.min_terrain_level),
    ...
    def _teleport_robots(self, env_ids, cfg):
        ...
        if cfg.terrain.teleport_robots:
            ...
            low_x_ids = robot_env_ids[
                self.root_states[robot_env_ids, 0] < thresh + x_offset
            ]
            ...
            high_x_ids = robot_env_ids[
                self.root_states[robot_env_ids, 0]
            ...
            high_y_ids = robot_env_ids[
                self.root_states[robot_env_ids, 1]
    ...
    def _init_buffers(self):
        ...
        self.rigid_body_state = gymtorch.wrap_tensor(rigid_body_state)[
            : self.num_envs * self.total_rigid_body_num, :
        ].view(self.num_envs, self.total_rigid_body_num, 13)[
            :, 0 : self.num_bodies, :
        ]  # .contiguous().view(self.num_envs*self.num_bodies,13)
        self.rigid_body_state_object = gymtorch.wrap_tensor(rigid_body_state)[
            : self.num_envs * self.total_rigid_body_num, :
        ].view(self.num_envs, self.total_rigid_body_num, 13)[
            :, self.num_bodies : self.num_bodies + self.num_object_bodies, :
        ]  # .contiguous().view(self.num_envs*self.num_bodies,13)
        ...
        self.foot_positions = self.rigid_body_state.view(self.num_envs, -1, 13)[
            :, self.feet_indices, 0:3
        ]
        self.prev_base_pos = self.base_pos.clone()
        self.prev_foot_velocities = self.foot_velocities.clone()
        ...
        self.contact_forces = gymtorch.wrap_tensor(net_contact_forces)[
            : self.num_envs * self.total_rigid_body_num, :
        ].view(self.num_envs, self.total_rigid_body_num, 3)[:, 0 : self.num_bodies, :]
        ...
        self.path_distance = torch.zeros(
        ...
        self.heading_commands = torch.zeros(
            self.num_envs, dtype=torch.float, device=self.device, requires_grad=False
        )  # heading
        self.heading_offsets = torch.zeros(
            self.num_envs, dtype=torch.float, device=self.device, requires_grad=False
        )  # heading offset
    ...
    def _init_custom_buffers__(self):
        ...
        self.ball_restitutions = self.default_restitution * torch.ones(
            self.num_envs, dtype=torch.float, device=self.device, requires_grad=False
        )
        self.ball_friction_coeffs = self.default_friction * torch.ones(
            self.num_envs, dtype=torch.float, device=self.device, requires_grad=False
        )
    ...
    def _prepare_reward_function(self):
        ...
        self.reward_container = reward_containers[
            self.cfg.rewards.reward_container_name
        ](self)
    ...
    def _create_envs(self):
        ...
        self.robot_asset, dof_props_asset, rigid_shape_props_asset = (
        ...
        self.object_init_state = to_torch(
            object_init_state_list, device=self.device, requires_grad=False
        )
        ...
        if self.cfg.env.add_balls:
            ...
            self.num_object_bodies = self.gym.get_asset_rigid_body_count(
                self.ball_asset
            )
        else:
        ...
        self.total_rigid_body_num = sum(
            [asset.get_num_bodies() for asset in all_assets]
        )
        self.num_dof = sum([asset.get_num_dof() for asset in all_assets])
        self.num_actuated_dof = sum(
            [asset.get_num_actuated_dof() for asset in all_assets]
        )

        if self.cfg.terrain.mesh_type == "boxes":
            self.total_rigid_body_num += (
                self.cfg.terrain.num_cols * self.cfg.terrain.num_rows
            )
        ...
        for i in range(self.num_envs):
            ...
            for bi in body_names:
                self.robot_rigid_body_idxs.append(
                    self.gym.find_actor_rigid_body_handle(env_handle, robot_handle, bi)
                )
            ...
            # add objects
            if self.cfg.env.add_balls:
                ball_rigid_shape_props = self._process_ball_rigid_shape_props(
                    ball_rigid_shape_props_asset, i
                )
                self.gym.set_asset_rigid_shape_properties(
                    self.ball_asset, ball_rigid_shape_props
                )
                ball_handle = self.gym.create_actor(
                    env_handle, self.ball_asset, self.ball_init_pose, "ball", i, 0
                )
                color = gymapi.Vec3(1, 1, 0)
                self.gym.set_rigid_body_color(
                    env_handle, ball_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, color
                )
                ball_idx = self.gym.get_actor_rigid_body_index(
                    env_handle, ball_handle, 0, gymapi.DOMAIN_SIM
                )
                ball_body_props = self.gym.get_actor_rigid_body_properties(
                    env_handle, ball_handle
                )
                ball_body_props[0].mass = self.cfg.ball.mass * (
                    np.random.rand() * 0.3 + 0.5
                )
                self.gym.set_actor_rigid_body_properties(
                    env_handle, ball_handle, ball_body_props, recomputeInertia=True
                )
                ...
                self.object_actor_idxs.append(
                    self.gym.get_actor_index(env_handle, ball_handle, gymapi.DOMAIN_SIM)
                )
        ...
        self.object_actor_idxs = torch.Tensor(self.object_actor_idxs).to(
            device=self.device, dtype=torch.long
        )
        self.object_rigid_body_idxs = torch.Tensor(self.object_rigid_body_idxs).to(
            device=self.device, dtype=torch.long
        )
        ...
        for i in range(len(feet_names)):
            self.feet_indices[i] = self.gym.find_actor_rigid_body_handle(
                self.envs[0], self.robot_actor_handles[0], feet_names[i]
            )
        ...
        for i in range(len(penalized_contact_names)):
            self.penalised_contact_indices[i] = self.gym.find_actor_rigid_body_handle(
                self.envs[0], self.robot_actor_handles[0], penalized_contact_names[i]
            )
        ...
        for i in range(len(termination_contact_names)):
            self.termination_contact_indices[i] = self.gym.find_actor_rigid_body_handle(
                self.envs[0], self.robot_actor_handles[0], termination_contact_names[i]
            )
        ################
        ### Add sensors
        ################
        ...
        if (
            self.cfg.perception.compute_segmentation
            or self.cfg.perception.compute_rgb
            or self.cfg.perception.compute_depth
        ):
    ...
    def _get_env_origins(self, env_ids, cfg):
        ...
        if cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            # put robots at the origins defined by the terrain
            max_init_level = int(
                cfg.terrain.max_init_terrain_level + cfg.terrain.num_border_boxes
            )
            min_init_level = int(
                cfg.terrain.min_init_terrain_level + cfg.terrain.num_border_boxes
            )
            if not cfg.terrain.curriculum:
                max_init_level = int(
                    cfg.terrain.num_rows - 1 - cfg.terrain.num_border_boxes
                )
            if not cfg.terrain.curriculum:
                min_init_level = int(0 + cfg.terrain.num_border_boxes)

            if cfg.terrain.center_robots:
                self.min_terrain_level = (
                    cfg.terrain.num_rows // 2 - cfg.terrain.center_span
                )
                ...
                self.terrain_types[env_ids] = (
                    torch.div(
                        ...
                        (
                            len(env_ids)
                            / (cfg.terrain.num_cols - 2 * cfg.terrain.num_border_boxes)
                        ),
                        rounding_mode="floor",
                    )
                    + cfg.terrain.num_border_boxes
                ).to(torch.long)
                self.min_terrain_level = int(cfg.terrain.num_border_boxes)
                self.max_terrain_level = int(
                    cfg.terrain.num_rows - cfg.terrain.num_border_boxes
                )
            cfg.terrain.env_origins[:, :, 2] = (
                self.terrain_obj.terrain_cell_center_heights.cpu().numpy()
            )
            cfg.terrain.terrain_origins = (
                torch.from_numpy(cfg.terrain.env_origins)
                .to(self.device)
    ...
    def _parse_cfg(self, cfg):
        ...
        cfg.domain_rand.ball_drag_rand_interval = np.ceil(
            cfg.domain_rand.ball_drag_rand_interval_s / self.dt
        )
    ...
    def get_segmentation_images(self, env_ids):
        ...
        for camera_name in self.cfg.perception.camera_names:
            segmentation_images = self.camera_sensors[
                camera_name
            ].get_segmentation_images(env_ids)
        return segmentation_images

    def get_rgb_images(self, env_ids):
        rgb_images = {}
        for camera_name in self.cfg.perception.camera_names:
            rgb_images[camera_name] = self.camera_sensors[camera_name].get_rgb_images(
                env_ids
            )
        return rgb_images
    ...
    def get_depth_images(self, env_ids):
        depth_images = {}
        for camera_name in self.cfg.perception.camera_names:
            depth_images[camera_name] = self.camera_sensors[
                camera_name
            ].get_depth_images(env_ids)
        return depth_images
    ...
    def initialize_cameras(self, env_ids):
        ...
        for camera_label, camera_pose, camera_rpy in zip(
            self.cfg.perception.camera_names,
            ...
            self.camera_sensors[camera_label].initialize(
                camera_label, camera_pose, camera_rpy, env_ids=env_ids
            )
    ...
    def set_lighting(self):
        ...
        self.gym.set_light_parameters(
            self.sim, light_index, intensity, ambient, direction
        )
