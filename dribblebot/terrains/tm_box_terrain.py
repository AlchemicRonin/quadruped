...
from dribblebot.utils.terrain import perlin
...
class TMBoxTerrain(Terrain):
    ...
    def prepare(self):
        ...
        self.terrain_cell_bounds = {
            "x_min": torch.zeros(
                self.env.cfg.terrain.num_rows,
                self.env.cfg.terrain.num_cols,
        ...
        min_restitution, max_restitution = (
            self.env.cfg.domain_rand.ground_restitution_range
        )
        self.terrain_cell_restitutions = (
            torch.rand(
                self.env.cfg.terrain.num_rows,
                self.env.cfg.terrain.num_cols,
                dtype=torch.float,
        ...
        lengthwise_density = int(
            self.env.cfg.terrain.terrain_length / self.env.cfg.terrain.horizontal_scale
        )
        widthwise_density = int(
            self.env.cfg.terrain.terrain_width / self.env.cfg.terrain.horizontal_scale
        )

        self.stair_heights_samples = (
            torch.zeros(
                self.env.cfg.terrain.num_rows,
                self.env.cfg.terrain.num_cols,
                dtype=torch.float,
            ...
            .repeat_interleave(lengthwise_density, dim=0)
            .repeat_interleave(widthwise_density, dim=1)
        )

        self.stair_runs_samples = (
            torch.zeros(
                self.env.cfg.terrain.num_rows,
                self.env.cfg.terrain.num_cols,
                dtype=torch.float,
        ...
        self.terrain_cell_heights = (
            ...
            * (self.env.max_height - self.env.min_height)
            + self.env.min_height
        )

        self.terrain_cell_center_heights = torch.zeros(
            self.env.cfg.terrain.num_rows,
            self.env.cfg.terrain.num_cols,
            dtype=torch.float,
            device=self.env.device,
        ...
        self.height_samples = (
            self.terrain_cell_heights.clone()
            .detach()
            .view(self.env.cfg.terrain.num_rows, self.env.cfg.terrain.num_cols)
            .repeat_interleave(lengthwise_density, dim=0)
            .repeat_interleave(widthwise_density, dim=1)
            .to(self.env.device)
            / self.env.cfg.terrain.vertical_scale
        )
        ...
        for i in range(self.env.cfg.terrain.num_rows):
            for j in range(self.env.cfg.terrain.num_cols):
                ...
                heightfield_segment_raw = self.env.terrain.height_field_raw[
                    start_px : end_px + 1, start_py : end_py + 1
                ]
                horizontal_scale = (
                    self.env.cfg.terrain.horizontal_scale
                )  # / self.env.cfg.terrain.num_rows
                vertical_scale = (
                    self.env.cfg.terrain.vertical_scale
                )  # / self.env.cfg.terrain.num_cols
                ...
                if self.env.terrain_props is not None:
                    self.terrain_cell_frictions[i, j] = self.env.terrain_props[
                        "friction"
                    ][i, j]
                min_friction, max_friction = (
                    self.env.cfg.domain_rand.ground_friction_range
                )
                ...
                tm_params.segmentation_id = int(
                    (tm_params.static_friction - min_friction)
                    / (max_friction - min_friction)
                    * 255
                )

                self.terrain_cell_bounds["x_min"][i, j] = (
                    i * self.env.terrain.cfg.env_width
                )
                self.terrain_cell_bounds["y_min"][i, j] = (
                    j * self.env.terrain.cfg.env_length
                )
                self.terrain_cell_bounds["y_min"][i, j] = (
                    j * self.env.terrain.cfg.env_length
                )
                self.terrain_cell_bounds["x_max"][i, j] = (
                    i + 1
                ) * self.env.terrain.cfg.env_width
                self.terrain_cell_bounds["y_max"][i, j] = (
                    j + 1
                ) * self.env.terrain.cfg.env_length
                ...
                if self.env.terrain_props is not None:
                    ...
                    if (
                        i < n_border_boxes
                        or i >= self.env.cfg.terrain.num_rows - n_border_boxes
                        or j < n_border_boxes
                        or j >= self.env.cfg.terrain.num_cols - n_border_boxes
                    ):
                        terrain_type = 0
                    ...
                    if self.env.cfg.terrain.curriculum:
                        terrain_type = (j - n_border_boxes) / (
                            self.env.cfg.terrain.num_cols - 2 * n_border_boxes
                        ) + 0.001
                        difficulty = (
                            (i - n_border_boxes)
                            / (self.env.cfg.terrain.num_rows - 2 * n_border_boxes)
                            * self.env.cfg.terrain.difficulty_scale
                        )
                    else:
                        terrain_type = np.random.random()
                        difficulty = (
                            self.env.cfg.terrain.difficulty_scale * np.random.random()
                        )
                    if (
                        i < n_border_boxes
                        or i >= self.env.cfg.terrain.num_rows - n_border_boxes
                ...
                if self.env.custom_heightmap is not None:
                    self.terrain_cell_center_heights[i, j] = self.env.custom_heightmap[
                        (start_px + end_px) // 2, (start_py + end_py) // 2
                    ]
                    self.height_samples[start_px:end_px, start_py:end_py] = (
                        self.env.custom_heightmap[start_px:end_px, start_py:end_py]
                        / self.env.cfg.terrain.vertical_scale
                    )
                else:
                    if terrain_type <= cume_props[0]:  # flat/rough
                        ...
                        if self.env.terrain_props is not None:
                            ...
                            step_run = (
                                self.env.cfg.terrain.max_step_run
                                - self.env.cfg.terrain.min_step_run
                            ) * torch.rand(1) + self.env.cfg.terrain.min_step_run
                            step_height = (
                                self.env.cfg.terrain.max_step_height * difficulty
                            )
                        num_steps = 8
                        for k in range(num_steps):
                            step_px = int(
                                step_run * k / self.env.cfg.terrain.horizontal_scale
                            )
                            if (
                                self.env.terrain.cfg.length_per_env_pixels
                                - 1.0 / self.env.cfg.terrain.horizontal_scale
                                <= step_px * 2
                            ):
                                k -= 1
                                break
                            ...
                            self.height_samples[
                                start_px + step_px : end_px - step_px,
                                start_py + step_px : end_py - step_px,
                            ] = height / self.env.cfg.terrain.vertical_scale
                            if k < num_steps - 1:
                                self.stair_heights_samples[
                                    start_px + step_px : end_px - step_px,
                                    start_py + step_px : end_py - step_px,
                                ...
                                self.stair_oris_samples[
                                    start_px : start_px + step_px,
                                    start_py + step_px : end_py - step_px,
                                ] = 0
                                self.stair_oris_samples[
                                    end_px - step_px : end_px,
                                    start_py + step_px : end_py - step_px,
                                ] = np.pi
                                self.stair_oris_samples[
                                    start_px + step_px : end_px - step_px,
                                    end_py - step_px : end_py,
                                ] = -np.pi / 2
                                self.stair_oris_samples[
                                    start_px + step_px : end_px - step_px,
                                    start_py : start_py + step_px,
                                ] = np.pi / 2
                            else:
                                self.stair_heights_samples[
                                    start_px + step_px : end_px - step_px,
                        ...
                        self.height_samples[start_px:end_px, start_py:end_py] = (
                            torch.tensor((max_height * xx * yy))
                        )

                        platform_size = 1.5

                        platform_size = int(
                            platform_size / self.env.cfg.terrain.horizontal_scale / 2
                        )
                        ...
                        self.terrain_cell_center_heights[i, j] = (
                            max_h * self.env.cfg.terrain.vertical_scale
                        )
                    elif terrain_type <= cume_props[4]:  # pit
                        self.height_samples[start_px:end_px, start_py:end_py] = (
                            -5.0 / self.env.cfg.terrain.vertical_scale
                        )
                        self.terrain_cell_center_heights[i, j] = 0

                # add perlin noise based on roughness
                if self.env.terrain_props is not None:
                    self.terrain_cell_roughnesses[i, j] = self.env.terrain_props[
                        "roughness"
                    ][i, j]
                roughness = self.terrain_cell_roughnesses[i, j]
                lin_x = np.linspace(
                    0,
                    self.env.cfg.terrain.terrain_length * 4,
                    self.env.terrain.cfg.width_per_env_pixels,
                    endpoint=False,
                )
                lin_y = np.linspace(
                    0,
                    self.env.cfg.terrain.terrain_width * 4,
                    self.env.terrain.cfg.length_per_env_pixels,
                    endpoint=False,
                )
                ...
                self.height_samples[start_px:end_px, start_py:end_py] = (
                    self.height_samples[start_px:end_px, start_py:end_py]
                    + torch.tensor(
                        perlin(x, y, seed=perlin_seed), device=self.env.device
                    )
                    * float(roughness)
                    / self.env.cfg.terrain.vertical_scale
                )
        ...
        self.friction_samples = (
            self.terrain_cell_frictions.clone()
            .detach()
            .view(self.env.cfg.terrain.num_rows, self.env.cfg.terrain.num_cols)
            .repeat_interleave(lengthwise_density, dim=0)
            .repeat_interleave(widthwise_density, dim=1)
        ...
        self.roughness_samples = (
            self.terrain_cell_roughnesses.clone()
            .detach()
            .view(self.env.cfg.terrain.num_rows, self.env.cfg.terrain.num_cols)
            .repeat_interleave(lengthwise_density, dim=0)
            .repeat_interleave(widthwise_density, dim=1)
    ...
    def initialize(self):
        ...
        self.env.terrain_actors = np.full(
            (self.env.cfg.terrain.num_rows, self.env.cfg.terrain.num_cols), None
        )
        ...
        for i in range(self.env.cfg.terrain.num_rows):
            for j in range(self.env.cfg.terrain.num_cols):
                terrain_px = (
                    i * self.env.terrain.cfg.env_width
                    + self.env.terrain.cfg.env_width / 2
                )
                terrain_py = (
                    j * self.env.terrain.cfg.env_length
                    + self.env.terrain.cfg.env_length / 2
                )

                border_px = int(
                    self.env.cfg.terrain.border_size
                    / self.env.cfg.terrain.horizontal_scale
                )
                ...
                end_py = (
                    border_px + (j + 1) * self.env.terrain.cfg.length_per_env_pixels
                )
                heightfield_segment_raw = np.array(
                    self.height_samples[
                        start_px : end_px + 1, start_py : end_py + 1
                    ].cpu(),
                    dtype=np.float16,
                ...
                self.env.gym.add_triangle_mesh(
                    self.env.sim,
        ...
        self.restitution_samples = (
            self.terrain_cell_restitutions.clone()
            .detach()
            .view(self.env.cfg.terrain.num_rows, self.env.cfg.terrain.num_cols)
            .repeat_interleave(lengthwise_density, dim=0)
            .repeat_interleave(widthwise_density, dim=1)
